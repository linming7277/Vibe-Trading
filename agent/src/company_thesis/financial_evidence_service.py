"""Convert verified Financial Agent Claims into immutable Thesis Evidence.

This service is deliberately a narrow bridge: it neither changes a Thesis nor
creates a Review.  It only writes auditable evidence for the Thesis version
that was current both when extraction started and immediately before writing.
"""
...
EXTRACTOR_VERSION = "company-thesis-financial-evidence-v1.0.0"
_EFFECTS = {"SUPPORT", "CHALLENGE", "NEUTRAL"}
_CANDIDATE_TYPES = {"FACT", "INFERENCE"}


class CompanyThesisFinancialEvidenceService:
    """Single-company Financial Claim extraction, with no batch entry point."""
    ...
    def __init__(
        ...
        *,
        ...
        self.financial_store = financial_store or FinancialAnalysisStore(self.evidence_service.repository.db_path)
    ...
    def _result(market: str, stock_code: str, **values: Any) -> dict[str, Any]:
        return {
            "market": market, "stock_code": stock_code, "status": "OK",
            "thesis_id": None, "financial_snapshot_id": None,
            "created": 0, "unchanged": 0,
            "skipped_forecast": 0, "skipped_unknown": 0,
            "skipped_low_confidence": 0, "skipped_invalid": 0,
            "evidence": [], **values,
        }

    @staticmethod
    def _source_keys(claim: dict[str, Any]) -> list[str]:
        raw = claim.get("source_keys", claim.get("evidence_keys", []))
    ...
    def _claim_text(claim: dict[str, Any]) -> str:
        return re.sub(r"\s+", " ", str(claim.get("text", claim.get("statement", "")) or "").strip())

    @staticmethod
    def _fingerprint(thesis_id: str, snapshot_id: str, claim: str, source_keys: list[str]) -> str:
        value = {
            "thesis_id": thesis_id,
            "financial_analysis_snapshot_id": snapshot_id,
    ...
    def _validate_relevance(result: dict[str, Any], expected_indexes: set[int]) -> dict[str, Any]:
        if set(result) != {"assessments"} or not isinstance(result.get("assessments"), list):
            raise ValueError("invalid relevance result schema")
        normalized: list[dict[str, Any]] = []
        indexes: set[int] = set()
        for item in result["assessments"]:
            if not isinstance(item, dict) or set(item) != {"claim_index", "effect", "reason"}:
                raise ValueError("invalid relevance assessment schema")
            ...
            if not isinstance(index, int) or index not in expected_indexes or index in indexes:
                raise ValueError("invalid relevance claim index")
            if effect not in _EFFECTS or not reason or len(reason) > 240:
                raise ValueError("invalid relevance effect or reason")
            indexes.add(index)
            normalized.append({"claim_index": index, "effect": effect, "reason": reason})
        if indexes != expected_indexes:
            raise ValueError("relevance assessments must cover every candidate exactly once")
    ...
    def _judge_relevance(self, thesis: dict[str, Any], candidates: list[dict[str, Any]]) -> tuple[dict[int, dict[str, str]] | None, dict[str, Any]]:
        direct: dict[int, dict[str, str]] = {}
        for candidate in candidates:
            existing = str(candidate["claim"].get("effect") or "").upper()
            if existing in _EFFECTS:
                direct[candidate["claim_index"]] = {
                    "effect": existing,
                    "reason": str(candidate["claim"].get("effect_reason") or "Financial Claim 已提供方向字段。")[:240],
                }
        if len(direct) == len(candidates):
            return direct, {"mode": "CLAIM_DIRECTION"}

        if self.relevance_resolver is not None:
            raw = self.relevance_resolver(thesis, candidates)
            parsed = self._validate_relevance({"assessments": raw}, {item["claim_index"] for item in candidates})
            return {item["claim_index"]: {"effect": item["effect"], "reason": item["reason"]}
                    for item in parsed["assessments"]}, {"mode": "INJECTED"}
        ...
        payload = {
            "current_thesis": {
                key: thesis.get(key) for key in ("thesis_id", "title", "core_thesis", "status", "confidence", "invalid_conditions")
            },
            "claims": [{
                "claim_index": item["claim_index"], "type": item["type"], "text": item["text"],
                "confidence": item["confidence"], "citations": item["citations"],
            } for item in candidates],
        }
        instruction = (
            "你只判断每条已验证财务 Claim 与当前 Company Thesis 的关系。"
            "SUPPORT=直接支持；CHALLENGE=直接反证或削弱；NEUTRAL=关联不足或方向不明。"
            "不得改写 Claim、不得生成新事实、不得给出交易建议。每条 reason 不超过 240 字，只依据输入。"
        ...
        def invoke(mode: StructuredOutputMode, response_format: dict[str, Any] | None) -> dict[str, Any]:
            ...
            if config.get("base_url") and callable(connection_invoke):
                return connection_invoke(
                    role="financial_analyst", phase="FINANCIAL_THESIS_RELEVANCE", model=config["model"],
            ...
            return self.runtime.invoke(
                role="financial_analyst", phase="FINANCIAL_THESIS_RELEVANCE", provider=config["provider"],
        ...
        outcome = self.structured_runtime.run(
            config=config, instruction=instruction, payload=payload,
            contract_schema=self._relevance_schema(len(candidates)), capabilities=capabilities,
            text_instruction="不可用：只返回纯文本。", text_payload={}, invoke_structured=invoke,
            validate=lambda value: self._validate_relevance(value, expected_indexes),
        )
        audit = {
            "mode": outcome.mode_used, "attempts": outcome.attempts, "error_types": outcome.error_types,
            "capability_source": outcome.capability_source,
        }
        if outcome.parsed is None:
            return None, audit
        return {item["claim_index"]: {"effect": item["effect"], "reason": item["reason"]}
                for item in outcome.parsed["assessments"]}, audit

    def extract_from_latest_financial_analysis(self, market: str, stock_code: str) -> dict[str, Any]:
        ...
        snapshot = self.financial_store.latest_completed(symbol)
        if snapshot is None:
            return {**result, "status": "FINANCIAL_ANALYSIS_NOT_READY"}
        result["financial_snapshot_id"] = snapshot["id"]
        ...
        if not bool(metadata.get("evidence_ready")):
            return {**result, "status": "CLAIMS_NOT_EVIDENCE_READY"}
        resolved = FinancialClaimCitationResolver().resolve_snapshot(
            snapshot, fallback_manifest=FinancialAnalysisService._evidence_manifest(snapshot),
        )
        ...
        for index, raw in enumerate(resolved_analysis.get("claims") or []):
            ...
            if (not text or not source_keys or not citations
            ...
            if confidence not in {"LOW", "MEDIUM", "HIGH"}:
                result["skipped_invalid"] += 1
                continue
            candidates.append({"claim_index": index, "claim": raw, "type": claim_type,
                               "confidence": confidence, "text": text, "source_keys": source_keys,
                               "citations": citations})
        if not candidates:
            return result
        ...
        for candidate in candidates:
            fingerprint = self._fingerprint(
                thesis["thesis_id"], snapshot["id"], candidate["text"], candidate["source_keys"],
            )
        ...
        # An idempotent retry must not call a model merely to rediscover an
        # already persisted effect. It is still fully auditable from Evidence.
        if not pending:
            return result
        ...
        for candidate in pending:
            ...
            evidence_metadata = {
                ...
                "claim_index": candidate["claim_index"], "claim_type": candidate["type"],
                "source_keys": candidate["source_keys"], "resolved_citations": candidate["citations"],
                "source_hashes": hashes, "financial_analysis_snapshot_id": snapshot["id"],
                "financial_prompt_version": metadata.get("prompt_version"),
                "financial_provider": metadata.get("provider") or snapshot.get("agent_provider"),
                "financial_model": metadata.get("model") or snapshot.get("agent_model"),
                "traceability_status": "COMPLETE", "relevance_reason": judgement["reason"],
            }
            try:
                evidence = self.evidence_service.create_evidence(
                    thesis_id=thesis["thesis_id"], evidence_type="FINANCIAL", effect=judgement["effect"],
                    claim=candidate["text"], summary=judgement["reason"], source_type="FINANCIAL_ANALYSIS",
                    source_id=snapshot["id"], source_ref=f"financial-analysis:{snapshot['id']}:claim:{candidate['claim_index']}",
                    source_title=f"Financial Analysis {symbol} Claim {candidate['claim_index']}",
                    source_date=snapshot.get("as_of"), data_as_of=snapshot.get("as_of"),
                    confidence=candidate["confidence"], created_by="AGENT_FINANCIAL",
                    metadata=evidence_metadata, evidence_fingerprint=fingerprint,
                )
...
_service: CompanyThesisFinancialEvidenceService | None = None


def get_company_thesis_financial_evidence_service() -> CompanyThesisFinancialEvidenceService:
    global _service
    if _service is None:
        _service = CompanyThesisFinancialEvidenceService()
    return _service

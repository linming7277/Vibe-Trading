...
function Conclusion({ data }: { data: CompanyResearchConclusion | null }) {
  ...
  return <div className="space-y-3"><div className="rounded-lg bg-muted/40 p-3 text-sm leading-6">{data.research_conclusion || "当前尚无可展示的一句话研究结论。"}</div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">当前逻辑</span><strong className="mt-1 block text-sm">{data.thesis ? thesisStatusLabel(data.thesis.status) : "尚未建立"}</strong></div><div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">价格关注条件</span><strong className="mt-1 block text-sm">{entryStatusLabel(data.entry.level)}</strong></div><div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">研究复核压力</span><strong className="mt-1 block text-sm">{exitStatusLabel(data.exit.level)}</strong></div><div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">研究证据</span><strong className="mt-1 block text-sm">支持 {data.evidence_counts.support} · 挑战 {data.evidence_counts.challenge}</strong></div></div><div className="grid gap-2 sm:grid-cols-2"><div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">合理价值区间</span><strong className="mt-1 block tabular-nums">{rangeText(data.fair_value_range)}</strong></div><div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">重点观察区</span><strong className="mt-1 block tabular-nums">{rangeText(data.focus_zone)}</strong></div></div></div>;
}

...
function ScorePanel({ scores, line, section }: { scores: EngineScore[]; line: StrategyLine; section: string }) {
  ...
  return <section className="overflow-hidden rounded-xl border bg-card shadow-sm"><div className="border-b px-5 py-4"><div className="text-xs font-semibold text-primary">DETERMINISTIC SCORE</div><h2 className="mt-1 text-lg font-semibold">确定性评分</h2></div><div className="grid grid-cols-[1fr_120px_120px_180px] gap-4 border-b bg-muted/30 px-5 py-3 text-xs font-medium text-muted-foreground"><span>对象</span><span>基础分</span><span>覆盖率</span><span>公式版本</span></div><div className="divide-y">{scores.map((score) => <Link key={score.id} to={score.subject_type === "security" ? `/company/${score.market}/${score.subject_id}` : line === "value" ? "/value" : "/emotion/sectors"} className="grid grid-cols-[1fr_120px_120px_180px] items-center gap-4 px-5 py-4 hover:bg-muted/40"><div><div className="font-medium">{score.subject_id}</div><div className="mt-1 text-xs text-muted-foreground">{score.status} · {score.data_as_of} · {section}</div></div><div>{score.base_score == null ? "—" : <ScoreBar value={score.base_score} compact />}</div><span className={cn("font-mono text-sm", score.coverage >= .8 ? "text-success" : "text-warning")}>{percent(score.coverage)}</span><span className="truncate font-mono text-xs text-muted-foreground">{score.formula_version}</span></Link>)}</div></section>;
}

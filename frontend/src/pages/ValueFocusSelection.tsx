...
import { api, type FocusSelectionItem, type FocusSelectionResponse, type StrategySignal, type ValueResearchAutomation } from "@/lib/api";
...
function signalTitle(signal: StrategySignal) {
  const direction = signal.direction.toUpperCase();
  return /(BUY|LONG|ENTRY|OPEN)/.test(direction) ? "入场候选" : /(SELL|EXIT|CLOSE|SHORT)/.test(direction) ? "风险 / 退出复核" : "需要复核";
}
...
export function ValueFocusSelectionPage() {
  ...
  const load = async () => {
    setLoading(true);
    const [focus, signal, auto] = await Promise.allSettled([api.getFocusSelection(), api.getValueSignals("CN"), api.getValueAutomation()]);
    if (focus.status === "fulfilled") setSelection(focus.value);
    if (signal.status === "fulfilled") setSignals(signal.value);
    if (auto.status === "fulfilled") setAutomation(auto.value);
    const failure = [focus, signal, auto].find((result) => result.status === "rejected");
  ...
  return <div className="mx-auto w-full max-w-[1250px] space-y-5 p-4 md:p-6"><header className="flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-semibold text-primary">LOW VALUE FOCUS</div><h1 className="mt-1 text-2xl font-semibold">机会与风险</h1></div><button type="button" onClick={() => void load()} disabled={loading} className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm hover:border-primary/50 disabled:opacity-60"><RefreshCw className={cn("h-4 w-4", loading && "animate-spin")} />刷新结果</button></header>{error ? <div className="rounded-lg border border-danger/30 bg-danger/10 p-3 text-sm text-danger">部分数据读取失败：{error}</div> : null}{loading ? <div className="flex h-72 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" />读取低估龙头研究优先级…</div> : <>{selection ? <><section className="grid gap-3 sm:grid-cols-4"><Metric label="当前低估龙头" value={`${selection.total_low_value} 家`} /><Metric label="重点研究" value={`${selection.A_count} 家`} /><Metric label="继续观察" value={`${selection.B_count} 家`} /><Metric label="暂缓优先研究" value={`${selection.C_count} 家`} /></section><TierSection title="重点研究" items={selection.A} open={open.A} onToggle={() => toggle("A")} /><TierSection title="继续观察" subtitle="有研究价值，但仍需观察风险、等待条件或补充资料；默认收起。" items={selection.B} open={open.B} onToggle={() => toggle("B")} /><TierSection title="暂缓优先研究" subtitle="仍属于当前低估龙头，只是暂不优先投入研究时间；默认收起。" items={selection.C} open={open.C} onToggle={() => toggle("C")} /><p className="rounded-lg bg-muted/40 p-3 text-xs leading-5 text-muted-foreground">{selection.selection_boundary}</p></> : <div className="rounded-xl border border-dashed bg-card p-8 text-center text-sm text-muted-foreground">当前没有可读取的低估龙头重点研究结果。</div>}<section className="rounded-xl border bg-card"><div className="flex items-center justify-between gap-3 border-b p-4"><div><h2 className="font-semibold">系统提醒</h2><p className="mt-1 text-sm text-muted-foreground">保留原有入场、风险和退出复核提醒；不与上方研究优先级混为同一种结论。</p></div><span className="text-xs text-muted-foreground">监控状态：{automation?.enabled ? "已启用" : "未启用"}</span></div><div className="divide-y">{currentSignals.slice(0, 12).map((signal) => <article key={signal.id} className="flex items-center justify-between gap-3 p-4"><div><strong className="text-sm">{signalTitle(signal)} · {signal.symbol}</strong><p className="mt-1 text-xs text-muted-foreground">数据日期 {signal.data_as_of}</p></div><Link to={companyResearchPath(signal.symbol, { from: "/value/opportunities", fromLabel: "机会与风险" }, "overview")} className="shrink-0 text-sm text-primary hover:underline">查看公司研究</Link></article>)}{!currentSignals.length ? <p className="p-6 text-center text-sm text-muted-foreground">当前没有已生成且有效的系统提醒。这不代表没有研究价值，只表示提醒规则未触发。</p> : null}</div></section></>}</div>;
}
